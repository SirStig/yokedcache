...
from .models import TableCacheConfig, InvalidationRule, InvalidationType, SerializationMethod
...
class CacheConfig:
    ...
    def _validate_config(self) -> None:
        ...
        if not (0 <= self.fuzzy_threshold <= 100):
            raise CacheConfigurationError("fuzzy_threshold", "must be between 0 and 100")
    ...
    if config.table_configs:
        ...
        for table_name, table_config in config.table_configs.items():
            ...
            if table_config.max_entries:
                config_dict["tables"][table_name]["max_entries"] = table_config.max_entries
